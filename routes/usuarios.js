...
usuarios
  ...
  .delete((req, res) => {
    ...
    if (!matricula) {
      // se não foi informado retorna um erro e encerra a requisição.
    ...
    // verifica se há resposta na pesquisa anterior
    if (!alunoEcontrado) {
      // se não existir envia mensagem de erro e encerra a requisição
      res.status(404).json({ mensagem: "Aluno não encontrado." });
      return;
    }
    ...
    /* 
        ...
        */
    const dbfiltrado = db.filter(
      (usuarios) => usuarios.matricula !== matricula
    );
